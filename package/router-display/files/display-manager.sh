...
config_get REFRESH_INTERVAL display refresh_interval "5"
...
cleanup() {
    ...
    [ -n "$TIMER_REFRESH_PID" ] && kill "$TIMER_REFRESH_PID" 2>/dev/null
...
# Refresh the dashboard while the screen is lit, so signal/traffic/battery
# do not sit frozen at whatever they were when the button was pressed.
TIMER_REFRESH_PID=""
...
# Boot sequence: splash first so the screen is never left showing the kernel
# console, then the dashboard as soon as the data has been gathered.
...
while read -r cmd <&3; do
    ...
    case "$cmd" in
        ...
        update)
            ...
            ;;
        qr)
            logger -t display "Show Wi-Fi QR"
            set_brightness $BRIGHTNESS_FULL
            start_timers
            [ -x $UPDATE_DISPLAY_BIN ] && $UPDATE_DISPLAY_BIN qr
            ;;
        shutdown)
